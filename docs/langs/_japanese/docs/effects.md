...
`画質`: 画質やフレームレートを設定します。また、`高画質 (High以上) のときもHDRをオフ`をオンにすると、HDRを使わずに描画します。HDRをオフにすると低負荷になりますが、明暗の表現が低品質になります。
...
`影`: 影の有効/無効、影の色、ぼかしの大きさ、濃さ、向き、奥行きを設定します。
...
`リムライト`: アバターの輪郭付近に光を当てるエフェクトです。リムライトの色、明るさ、適用率、リムの厚さ、角度を設定します。

`Ambient Occlusion`: アバターのくぼみやパーツの境界付近を暗くするエフェクトです。エフェクトの有効/無効と強さを設定します。
...
デフォルト設定では、[ゲーム入力](../game_input)機能が有効であるか[VMC Protocol](../vmc_protocol)の受信によって下半身の姿勢を受信しているときに下半身モーションを制御中であると判定し、全身表示用シャドウが表示されます。`つねに全身表示用シャドウを適用`をオンにすると、下半身モーションの状態によらず全身表示用シャドウが表示されます。
