...
### 高度な機能: VRMファイルのロード時の設定
{: .doc-sec2 }

本機能はv4.5.0およびそれ以降のバージョンで利用できます。

設定ウィンドウの`ファイル`タブで、`VRMファイルの操作`メニューから、VRMファイルロード時のライセンス確認をスキップするオプションが指定できます。

本オプションを有効化した場合、PC上の `.vrm` ファイルを読み込む場合の挙動が以下のようになります。

<div class="doc-ul" markdown="1">

- ファイルダイアログで `.vrm` ファイルを選択してロードするとき、ライセンスUIを表示せず、直ちにモデルをロードします。
- 同様に、コントロールパネルの`ホーム` タブに `.vrm` をドラッグ & ドロップした場合も、直ちにモデルをロードします。
...
本オプションは次節の「オートメーション」オプションと同様、ライセンスが十分管理されたVRMのみを使用する場合にのみ使用して下さい。

ライセンスが未確認のモデルを新たにロードしたい場合、本オプションを無効にして下さい。

VRoid Hubのモデルに対しては本オプションのオン/オフによらず、常にライセンス確認が必要です。
